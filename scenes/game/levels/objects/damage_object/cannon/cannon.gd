...
var desired_y_position = 20.0  # La posición Y en la que quieres mantener el objeto
var gravity_force = 20  # Fuerza gravitacional personalizada (ajustar según sea necesario)
func _process(delta):
	# Establecer la velocidad vertical en 0 para mantener el objeto en el aire
	if position.y > desired_y_position:
		linear_velocity.y = -gravity_force
	elif position.y < desired_y_position:
		linear_velocity.y = gravity_force
	else:
		linear_velocity.y = 0
